...
export function Navbar({
  ...
  return (
    <nav
      {...props}
      className={clsxMerge(className, 'flex flex-1 items-center gap-4 py-2.5')}
    />
  );
}
...
export function NavbarDivider({
  ...
  return (
    <div
      ...
      className={clsxMerge(
        className,
        'h-6 w-px bg-teal-950/10 dark:bg-white/10',
      )}
    />
  );
}
...
export function NavbarSection({
  ...
  return (
    <LayoutGroup id={id}>
      <div
        {...props}
        className={clsxMerge(className, 'flex items-center gap-3')}
      />
    </LayoutGroup>
  );
}
...
export function NavbarSpacer({
  ...
  return (
    <div
      aria-hidden="true"
      {...props}
      className={clsxMerge(className, '-ml-4 flex-1')}
    />
  );
}
...
export const NavbarItem = forwardRef(function NavbarItem(
  ...
  let classes = clsxMerge(
    // Base
    'relative flex min-w-0 items-center gap-3 rounded-lg p-2 text-left text-base/6 font-medium text-teal-950 sm:text-sm/5',
    ...
    <span className={clsxMerge(className, 'relative')}>
      {current && (
        <motion.span
          ...
          className="absolute inset-x-2 -bottom-2.5 h-0.5 rounded-full bg-teal-950 dark:bg-white"
        />
      )}
      {'href' in props ? (
        <Link
