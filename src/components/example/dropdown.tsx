...
export function DropdownItem({
  ...
  let classes = clsxMerge(
    ...
    // Text styles
    'text-left text-base/6 text-teal-950 sm:text-sm/6 dark:text-white forced-colors:text-[CanvasText]',
    // Focus
    ...
    '[&>[data-slot=icon]]:text-teal-500 [&>[data-slot=icon]]:data-[focus]:text-white [&>[data-slot=icon]]:dark:text-teal-400 [&>[data-slot=icon]]:data-[focus]:dark:text-white',
    // Avatar
...
export function DropdownHeader({
  ...
  return (
    <div
      {...props}
      className={clsxMerge(className, 'col-span-5 px-3.5 pb-1 pt-2.5 sm:px-3')}
    />
  );
}
...
>) {
  return (
    <Headless.MenuHeading
      ...
      className={clsxMerge(
        className,
        'col-span-full grid grid-cols-[1fr,auto] gap-x-12 px-3.5 pb-1 pt-2 text-sm/5 font-medium text-teal-500 sm:px-3 sm:text-xs/5 dark:text-teal-400',
      )}
    />
  );
}
...
>) {
  return (
    <Headless.MenuSeparator
      ...
      className={clsxMerge(
        className,
        'col-span-full mx-3.5 my-1 h-px border-0 bg-teal-950/5 sm:mx-3 dark:bg-white/10 forced-colors:bg-[CanvasText]',
      )}
    />
  );
}
...
export function DropdownLabel({
  ...
  return (
    <Headless.Label
      {...props}
      data-slot="label"
      className={clsxMerge(className, 'col-start-2 row-start-1')}
      {...props}
    />
  );
}
...
>) {
  return (
    <Headless.Description
      ...
      className={clsxMerge(
        className,
        'col-span-2 col-start-2 row-start-2 text-sm/5 text-teal-500 group-data-[focus]:text-white sm:text-xs/5 dark:text-teal-400 forced-colors:group-data-[focus]:text-[HighlightText]',
      )}
    />
  );
}
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split('')).map((char, index) => (
        <kbd
          key={index}
          className={clsxMerge([
            'min-w-[2ch] text-center font-sans capitalize text-teal-400 group-data-[focus]:text-white forced-colors:group-data-[focus]:text-[HighlightText]',
